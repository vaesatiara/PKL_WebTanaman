    ...
    try {
        ...
        if ($stmt_detail) {
            foreach ($sessionData['order_items'] as $item) {
                if (!isset($item['id_produk']) || !isset($item['jumlah']) || !isset($item['harga']) || !isset($item['subtotal'])) {
                    continue;
                }
                
                $stmt_detail->bind_param("iiidd", $id_pesanan, $item['id_produk'], $item['jumlah'], $item['harga'], $item['subtotal']);
                $stmt_detail->execute();
            }
        }
